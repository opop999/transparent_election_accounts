...
for (i in seq.int(from = 0, to = 20700, by = 50)) {
  
  new_lines <- fromJSON(content(GET(url = paste0("https://www.kb.cz/transparentsapi/transactions/4070217?skip=", i)), as = "text"))[[2]]
  
  # Replace with list
  
  full_df <- bind_rows(full_df, new_lines) %>% distinct()
  
  Sys.sleep(runif(1, min = 0.1, max = 0.2))
  
}

saveRDS(full_df, "../data/temporary_manual_scrape/ano_2011_donation.rds")

full_df <- readRDS("../data/temporary_manual_scrape/ano_2011_donation.rds")
...
# FOR CS
full_df <- fromJSON("data/temporary_manual_scrape/top09_donation.txt", flatten = TRUE)

full_df <- full_df$transactions

# FOR CSOB
full_df3 <- fromJSON("data/temporary_manual_scrape/kscm_donation_3.txt", flatten = TRUE)

full_df3 <- full_df3$accountedTransactions$accountedTransaction

full_df_combined <- bind_rows(full_df, full_df2, full_df3)


fwrite(full_df, "../data/temporary_manual_scrape/csv/ano_2011_donation.csv")
